...
func (s *Service) initAutocertTlsDebugServer() {
	cfg, err := s.Config()
	if err != nil {
		s.log.Error("getting config: %v", err)
		panic(err)
	}
	...
	if err != nil {
		s.log.Error("simplecert init failed: %v", err)
		panic(err)
	}
	...
	s.log.Info("serving", "address", "https://"+certConfig.Domains[0])
...
func (s *Service) initAutocertTlsProductionServer() {
	...
	// start serving
	s.log.Info("serving", "address", fmt.Sprintf("https://%s", certConfig.Domains[0]))
	serve(ctx, srv)
}
